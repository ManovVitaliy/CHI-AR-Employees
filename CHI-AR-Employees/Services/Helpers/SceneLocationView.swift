...
//    public var showAxesNode = false
    ...
    public var didFetchInitialLocation = false
...
//        delegate = self
    ...
    public func run() {
        ...
        updateEstimatesTimer = Timer.scheduledTimer(timeInterval: 0.5, target: self, selector: #selector(SceneLocationView.updateLocationData), userInfo: nil, repeats: true)
    }
    ...
    public func updatePositionAndScaleOfLocationNode(locationNode: LocationNode, initialSetup: Bool = false, animated: Bool = false, duration: TimeInterval = 0.3) {
        guard let currentPosition = currentScenePosition(),
